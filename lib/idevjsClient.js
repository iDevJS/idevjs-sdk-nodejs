...
export default class IDevJSClient {
   ...
   * @return {string}
   */
  getAuthorizationUrl(redirectUrl, requestedScope, state) {
    return url.format({
      protocol: 'https',
      host: BASE_URI,
  ...
  _makeRequest(options, callback) {
    return new Promise((_resolve, _reject) => {
      ...
      var requestParams = {
        ...
        path: options.auth ? `${options.path}` : `/${this._version}${options.path}`
      }
      var req = https.request(requestParams, (res) => {
        ...
        res.on('end', () => {
          ...
          try {
            ...
            reject(new IDevJSError('Failed to parse response', DEFAULT_ERROR_CODE), null)
            return
          }
          ...
          if (statusType == 4 || statusType == 5) {
            ...
            reject(new IDevJSError(err.message, err.code), null)
          } else if (statusType == 2) {
            resolve(null, payload.data || payload)
          } else {
            reject(new IDevJSError('Unexpected response', DEFAULT_ERROR_CODE), null)
          }
        })
      }).on('error', function (err) {
        reject(new IDevJSError(err.message, DEFAULT_ERROR_CODE), null)
      })
